<template>
  <article class="label" :style="style">
    ...
    <div v-if="price.breakdown.length > 0" class="label__breakdown">
      <div
        v-for="(row, index) in price.breakdown"
        :key="index"
        class="label__row"
...
const props = defineProps<{
  label: Label
  /**
   * What measuring found: fitted font sizes in pixels, keyed by style, and how
   * the add-on rows came out. Optional: without it the stylesheet falls back to
   * its defaults, so a label still renders correctly before measuring has run.
   */
  fit?: LabelFit
...
const rowHeights = computed(() => breakdownRowHeightsCss(content.value, props.labelStyle ?? DEFAULT_LABEL_STYLE))
...
const style = computed<Record<string, string>>(() => {
  ...
  for (const [key, cssVar] of Object.entries(FONT_SIZE_VARS) as [StyleKey, string][]) {
    const size = props.fit?.sizes[key]
